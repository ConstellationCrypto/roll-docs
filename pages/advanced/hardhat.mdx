...
It can directly interact with Form's Ethereum API, allowing for the deployment of smart contracts into the Form network.
...
4. Modify the `hardhat.config.js` file
    ...
    - Inside the `module.exports` add the Form network configuration 
    ...
    networks: {
        ...
        form: {
            url: "https://rpc.form.network/http",  // form RPC URL Here
            chainId: 1748, // form ChainID Here
        }
    },
    }
    ```
...
4. Deploy `your_contract.sol` using the command below
    ```
    npx hardhat run scripts/deploy.js --network form
    ```