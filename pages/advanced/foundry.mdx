...
Additionally, Foundry can directly communicate with Form's Ethereum API, enabling the use of Foundry to deploy smart contracts into the Form network.
...
To deploy the `MyToken.sol` contract, use the command that corresponds to the Form chain's RPC URL while running the `forge create` command:
```
forge create --rpc-url "https://rpc.form.network/http"
